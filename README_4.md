...
|------|-------------|---------|
| Supreme Commander | Presence (Leadership) | Command Bridge & Auspex Station |
| Enginseer | Tech (Engineering) | Reactor Status |
| Helmsman | Reflexes (Major Voidship) | Navigation Bridge |
| Weapons Master | Ranged (Ordnance) | Ordnance Deck |

---

## Supreme Commander / Auspex Officer
...
The Supreme Commander also operates the **Auspex Radar**: an interactive canvas overlay plotting all ship tokens as blips. The primary task is building and maintaining **Sensor Locks** on enemy contacts.
...
After the Gunner fires, the Supreme Commander may perform one BDA correction (rolled with Leadership):
...
18-card deck shuffled at the start of combat. Draw 3 cards per round (modified by Inspire SL). Hand cap is 6 (modified by Inspire SL for that round). One free redraw per round (swap one card for a fresh draw). Spending two repair actions unlocks a full mulligan.

Boost cards targeting the Auspex Officer role (Enhanced Auspex, Sensor Priority) are not included in the four-player deck, as auspex duties are handled by the Supreme Commander.
...
The Weapons Master fires the ship's weapon batteries and manages crew logistics and ordnance deployment. Hostiles can only be targeted and damage can only be assessed with the assistance of the Supreme Commander.
