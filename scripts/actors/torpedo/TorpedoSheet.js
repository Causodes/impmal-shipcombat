...
export class TorpedoSheet extends IMActorSheet {
  ...
  async _prepareContext(options) {
    ...
    const isRealisticMode = game.settings?.get(MODULE_ID, "movementMode") === "realistic";
    const velocityMagCtx  = isRealisticMode ? Math.floor(Math.hypot(helm.velocityX ?? 0, helm.velocityY ?? 0)) : 0;
    const minMove = isRealisticMode ? velocityMagCtx : Math.ceil(speed / 2);
    ...
    context.helm = {
      ...
      isRealistic:          game.settings?.get(MODULE_ID, "movementMode") === "realistic",
      velocityX:            helm.velocityX ?? 0,
      velocityY:            helm.velocityY ?? 0,
      // bearingUsed intentionally always 0 for ordnance: torpedoes use linked actors,
      // so actor-level accumulation would contaminate all tokens of the same template.
      // The budget bar simply shows the current confirm's bearing vs the max.
      bearingUsed:          0,
      momentumUsed:         0,
      bearingRemaining:     mano * 15,
      bearingBudgetTooltip: (() => {
        const isR = game.settings?.get(MODULE_ID, "movementMode") === "realistic";
        if (!isR || typeof game === "undefined") return "";
        const bMax = mano * 15;
  ...
  _onRender(context, options) {
    ...
    const bearingMax    = context.helm.maxBearing;    // mano * 15
    ...
    if (carryInput) {
      ...
      if (context.helm.designated) carryInput.disabled = true;
  ...
  static async _onConfirmHelm() {
    ...
    const updates = {
      ...
      // bearingUsed and momentumUsed are NOT saved for ordnance: torpedoes use
      // linked actors so saving to the actor would contaminate all tokens of the
      // same template actor.  The budget bar is display-only per confirm.
    };
